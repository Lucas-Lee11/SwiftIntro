...
struct Spirograph: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        for theta in stride(from: 0, through: endPoint, by: 0.01) {
            var x:CGFloat = difference * cos(theta) + distance * cos(difference / outerRadius * theta)
            var y:CGFloat = difference * sin(theta) - distance * sin(difference / outerRadius * theta)
